...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
    ...
    Vector2D pFilm(2.0 * (x - 0.5) * tan(radians(hFov) * 0.5), 2.0 * (y - 0.5) * tan(radians(vFov) * 0.5));
    Vector3D pLens = lensRadius * Vector3D(sqrt(rndR) * cos(rndTheta), sqrt(rndR) * sin(rndTheta), 0.0);
    Vector3D dCentral = Vector3D(pFilm.x, pFilm.y, -1.0).unit();
    double tFocus = focalDistance / fabs(dCentral.z);
    Vector3D pFocus = dCentral * tFocus;

    Vector3D dRay = (pFocus - pLens).unit();

    Ray r(pos + c2w * pLens, c2w * dRay);
    r.min_t = nClip; r.max_t = fClip;
    return r;
}
